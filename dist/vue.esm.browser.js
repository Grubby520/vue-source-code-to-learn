 ...
 */
class Dep {
   // 静态属性：全局唯一的 Watcher (nice: 同一时间只能有一个全局的Watcher实例被计算)
...
const arrayMethods = Object.create(arrayProto); // 继承, 创建对象，传入的是Array.prototype作为新建对象的原型对象
...
const seenObjects = new _Set(); // new caches to set dep.id
...
// 全局变量
let activeInstance = null; // 保持当前vue的执行上下文
 ...
 * 差异化部分只需要通过参数来区分。
 */
const patch = createPatchFunction(params);
  ...
  while (html) {
    ...
    if (!lastTag || !isPlainTextElement(lastTag)) {
      ...
      if (textEnd === 0) {
        ...
        // End tag:
        const endTagMatch = html.match(endTag$1);
        if (endTagMatch) {
